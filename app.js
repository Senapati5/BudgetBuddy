...
const userSchema = new mongoose.Schema({
    ...
    mobile: { type: String, required: true } // Store mobile number
});
...
app.post('/api/signup', upload.single('profile-picture'), async (req, res) => {
    const { name, username, password, email, mobile } = req.body; // Add email and mobile
    const profilePicture = req.file ? req.file.path : null; // Store file path if uploaded
    const hashedPassword = await bcrypt.hash(password, 10);
    
    const user = new User({ name, username, password: hashedPassword, profilePicture, email, mobile });
